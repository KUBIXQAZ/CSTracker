...
namespace SteamItemsStatsViewer.Commands
{
    public class RefreshDataCommand : CommandBase
    {
        ...
        public override void Execute(object? parameter)
        {
            ...
            _iSeriesPrice[0].Values = _itemData.PriceHistory.Where(x => DateTime.Now.Date.AddDays(-(int)_priceChartTimeStamp) <= x.Key.Date).Select(x => (double)Math.Round(x.Value * rate, 2));
            _xAxesPrice[0].Labels = _itemData.PriceHistory.Where(x => DateTime.Now.Date.AddDays(-(int)_priceChartTimeStamp) <= x.Key.Date).Select(x => x.Key.ToString()).ToArray();

            _xAxesPrice[0].MinLimit = 0;
            _xAxesPrice[0].MaxLimit = _xAxesPrice[0].Labels.Count - 1;

            _iSeriesQuantity[0].Values = _itemData.QuantityHistory.Where(x => DateTime.Now.Date.AddDays(-(int)_quantityChartTimeStamp) <= x.Key.Date).Select(x => x.Value);
            _xAxesQuantity[0].Labels = _itemData.QuantityHistory.Where(x => DateTime.Now.Date.AddDays(-(int)_quantityChartTimeStamp) <= x.Key.Date).Select(x => x.Key.ToString()).ToArray();
            ...
            _viewModel.CurrentPrice = $"{(_itemData.PriceHistory.Last().Value * rate).ToString("N")}{App.Settings.Currency}";

            _viewModel.CurrentQuantity = _itemData.QuantityHistory.Last().Value.ToString("N0");
        }
